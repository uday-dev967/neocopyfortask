const initialState = () => ( {
    ...
    //     </svg>`,
		// 	primaryMessage: "message comes here",
		// 	secondaryMessage: "secondary message here",
	...
	actions: {
		addNotification( { commit, state, dispatch }, notification ) {
			...
			if ( ! notification.isPersistent ) {
				...
				state.notificationTimeoutRefTracker[ id ] = setTimeout( () => {
		...
		removeNotification( { commit, state }, notification ) {
			...
			commit( "setNotifications", state.notifications.filter( e => e.id !== notification.id ) );
			delete state.notificationTimeoutRefTracker[ notification.id ];
