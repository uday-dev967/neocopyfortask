<template>
	<div class="popup-div showPopUp">
        ...
        <div class="popup-head">
          <h2 class="creat-project-title">{{ $t("cancelSubIndex.cancelSubscriptionHeader") }}</h2>
        </div>
        <div class="popup-details">
          <div class="body">
            {{ $t("cancelSubIndex.cancelSubscriptionBody[0]") }} {{ expiryDate }} {{ $t("cancelSubIndex.cancelSubscriptionBody[1]") }}
          </div>
          <div class="invite-button-container">
            <button  class="cancel-button" @click="handleConrimClick">{{ $t("buttonText.confirm") }}</button>
          </div>
        </div>
      </div>
	</div>
...
export default {
  ...
  methods: {
    ...
    handleConrimClick() {
      console.log("handleConrimClick did ")
      this.$emit("on-confrim")
    }
