<template>
	<div class="popup-div" :class="showMoodboardCancelConfirmationPopup ? 'showPopUp' : ''">
		<div class="popup-container moodboard-signup-popup">
			<div class="close-popup" @click.stop="handleNoButtonClick">
				<svg
					width="24"
					height="24"
					viewBox="0 0 14 14"
            ...
            <div class="popup-body">
                <div class="confirmation-prompt-wrapper">
                    <div class="image-wrapper">
                        <img src="../../../assets/images/cancelsub/cancel-confirmation-prompt.png" alt="">
                    </div>
                    <div class="confirmation-prompt-text">
                        Are you sure you want to cancel the subscription?
                    </div>
                </div>
                <div class="popup-action-wraper">
                    <button @click="handleYesButtonClick">Yes</button>
                    <button @click="handleNoButtonClick">No</button>
                </div>
            </div>
		</div>
	</div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Dash", [
      "showMoodboardCancelConfirmationPopup",
  ...
  methods: {
    ...
    ...mapMutations({
      setShowMoodboardCancelConfirmationPopup:
        "Dash/setShowMoodboardCancelConfirmationPopup",
      setCancellationPopupButtonCB: "Dash/setCancellationPopupButtonCB",
      setSwitchToMoodboardAndRedirectToDashboard: "User/setSwitchToMoodboardAndRedirectToDashboard"
    }),
    handleYesButtonClick() {
      if (
        this.cancellationPopupButtonCB &&
        this.cancellationPopupButtonCB.postConfirmationCallback
      ) {
        this.cancellationPopupButtonCB.postConfirmationCallback();
      }
      this.setShowMoodboardCancelConfirmationPopup(false);
      this.setCancellationPopupButtonCB({
...
.moodboard-signup-popup {
  ...
  .popup-body {
    ...
    .confirmation-prompt-wrapper {
      padding: 0 2.49rem 0 5.49rem;
      background: #faf5f2;
      display: flex;
      justify-content: center;
      align-items: center;
      height: 21.375rem * 1.2;
    ...
    .popup-action-wraper {
      ...
      button {
        cursor: pointer;
        padding: 0.4375rem * 1.22 5rem * 1.22;
        // background: #ffffff;
        border: 1px solid $color-second;
        position: relative;
        box-shadow: 0px 0px 1px rgba(40, 41, 61, 0.04),
          0px 2px 4px rgba(96, 97, 112, 0.16);
          @include background-animate($color-second, black);
        // border-radius: 8px;
