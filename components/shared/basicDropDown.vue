	...
	>
		...
		<div ref="dropDownContainer" class="drop-down-container" :style="{ top: border ? '101%' : '100%' }">
			<div
				...
				:key="i"
				:class="{ selected: item.value ? selectedItem == item.value : selectedItem == item }"
				@click.stop="listElementClickHandler(item)"
			>
				<p class="noselect" v-html="item.text ? item.text : item"></p>
			</div>
	...
	export default {
			...
			displayTextForSelectedValue() {
				if (typeof this.dropDownList[0] == "object") {
					for (let option of this.dropDownList) {
						if (option.value === this.selectedItem) {
							return option.text;
						}
					}
				}
		...
		methods: {
			listElementClickHandler(item) {
				...
				this.$emit("listElementClick", this.selectedItem);
