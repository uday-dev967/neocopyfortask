		...
		>
			<div class="popup-container" @click.stop>
				...
				<div class="popup-details">
					...
					<AspectRatioSelection @aspectRatioChange="handleARChage"></AspectRatioSelection>
...
export default {
	...
	computed: {
		...mapGetters("moodboardDashboard", ["newProject", "message"]),
	},
	methods: {
		...mapMutations("moodboardDashboard", ["setNewProject", "setMessage"]),
		...mapActions("moodboardDashboard", ["addNewMbProject"]),
		...
		addNewProject() {
			if (this.checkSpecialCharacters(this.projectName)) {
				this.setMessage({
					value:
						"Name can contain only alphanumeric characters, spaces and underscores.",
					error: true,
					global: false,
				});
				return;
			}
			this.addNewMbProject({
				name: this.projectName,
				canvasDimension: this.selectedAspectRatio
					? this.getHeightAndWidth(this.selectedAspectRatio)
					: {
							height: 900,
							width: 1600,
					  },
