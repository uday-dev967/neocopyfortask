...
export default {
	...
	computed: {
		...mapGetters("moodboardDashboard", ["showRename", "message"]),
	},
	methods: {
		...mapMutations("moodboardDashboard", ["setshowRename", "setMessage"]),
		...mapActions("moodboardDashboard", ["renameMBProject"]),
		...mapActions("Moodboard", ["fetchProjects"]),
		closePopup() {
			this.setshowRename(false);
			this.newProjectName = "";
			this.setMessage({
				value: "",
				error: false,
		...
		onClickRename(name) {
			if (this.checkSpecialCharacters(name)) {
				this.setMessage({
			...
			this.renameMBProject({ name: name })
				.then(res => {
