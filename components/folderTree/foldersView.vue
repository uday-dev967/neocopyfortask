<template>
  <div>
    <div class="container" :class="{'search-folder': $parent.searchKey}">
      <div
        class="new-folder"
        v-for="(folder, index) in foldersArray.folders"
        v-if="!move"
        @mouseover="folderId = folder._id;"
        @mouseleave="folderId = null; showFolderMenu=false"
      >
        <svg
          width="17"
          height="18"
        ...
        >
          <path
            d="M0.952148 5.71973V17.0811H15.9437V8.54325H7.40593L5.25468 5.71973H1.69165H0.952148Z"
            stroke="#3A393A"
            stroke-width="0.8"
          ...
          />
          <path
            d="M2.76715 5.24912V1.08105H14.868V7.33316V8.54324"
            stroke="#3A393A"
            stroke-width="0.8"
            stroke-miterlimit="10"
            stroke-linecap="round"
        ...
        <div class="folder-menu" v-if="((folderId === folder._id) && showFolderMenu)">
          ...
          <div @click="showDelete=true; deleteFolderId= folder._id; parentId = folder.parent;message.value ='';message.global = false;">
      ...
      <div
        class="noFolders"
        v-if="(foldersArray && foldersArray.folders && foldersArray.folders.length <= 0 && !move)"
      >
    ...
    <!-- ---------------------------------MOVE POPUP--------------------------- -->
    <div
      class="folder-container folder-container-move"
      v-if="(foldersPopup && foldersPopup.folders && foldersPopup.folders.length > 0)"
    >
      <div
        class="folder"
        style="width: 286px !important;"
        v-for="folder in (foldersPopup && foldersPopup.folders)"
    ...
    <div
      class="noFolder-move"
      v-if="foldersPopup && foldersPopup.folders && foldersPopup.folders.length <= 0 && move"
    >
...
export default {
  ...
  watch: {
    foldersArray: function() {
      // this.$parent.breadcrumbs.push(this.foldersArray)
      console.log("---BC---", this.$parent.breadcrumbs);
      console.log("---FolderArray---", this.foldersArray);
    }
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    selectFolder(_folders) {
      ...
      if(this.$parent.searchKey){
        ...
        console.log('FOLDER SEELECTED', _folders)
        // Click on child folder, so searching for parents for breadcrumbs---->
        if(_folders && _folders.ancestors && _folders.ancestors.length > 0){
          _folders.ancestors.forEach((parentId)=> {
            let tempBC = this.foldersTree.folders.filter(folder => {
              //  searching for parents for breadcrumbs---->
              this.searchParent(folder,parentId,'search',_folders)
              });
          })
        }else {
          let tempTree = this.foldersTree.folders.filter(folder => folder._id === _folders._id);
          if(tempTree && tempTree.length > 0)
          this.$parent.rootFolders = tempTree[0];
          this.$parent.breadcrumbs.push(tempTree[0]);
        }
        this.$parent.searchKey = '';
      }else {
        this.$parent.rootFolders = _folders;
      ...
      this.fetchFolders({}).then(foldersList => {
        if (foldersList) {
          if (_parendId) {
            ...
            // this.$parent.breadcrumbs.push(foldersList);
          }
