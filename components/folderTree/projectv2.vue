  ...
  <div class="assistance" @click.prevent="isFilterDropdown = false;">
    ...
    <!-- <foldersView :folders-array="rootFolders"></foldersView> -->
      <div class="projects-cards-header">
        <div>
          <h2 v-if="!$parent.searchKey">{{dropdownFilterValue === 'projects' ? 'My Projects': 'Shared with me'}}</h2>
          <p v-if="dropdownFilterValue === 'projects' && !searchKey" class="sub-text">Create, access and edit your designs here</p>
        </div>
        <!-- <div class="filter-dropdown" v-if="breadcrumbs && breadcrumbs.length <= 0 && !searchKey" @click.stop>
          <span @click="isFilterDropdown = !isFilterDropdown">
            <p>{{dropdownFilterValue === 'shared' ? 'Shared with me': 'My Projects'}}</p>
          ...
          <ul v-if="isFilterDropdown">
              <li @click="dropdownSelected('project')">My Projects</li>
              <li @click="dropdownSelected('shared')">Shared with me</li>
          </ul>
       </div> -->
      </div>
    <!-- <h2>My Projects</h2>
    ...
    <div class="cards">
      ...
      <div
        ...
        v-for="(beg, index) in projects" :key="index"
        :class="{'sampleProject':beg.isSampleProject}"
        @mouseover="projectId = beg._id"
        @mouseleave="projectId=''">
        <img
          v-on:click="(beg && beg.isTemplate) ? cloneSampleProject(beg, 'open') : openProject(beg._id,beg.role)"
          v-if="beg.properties"
          class="main-image"
        ...
        <div class="card-text">
          ...
          <div class="card-text-head">
            ...
            <div>
              ...
              <!-- <svg v-bind:class="{'block-btn': !checkingPermission(beg,'share')}" v-if="dropdownFilterValue ==='projects'" @click="(beg && beg.isTemplate) ? cloneSampleProject(beg, 'share'): isClickable(beg,'share')" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="no-shared" v-if="dropdownFilterValue === 'shared' && foldersArray.sharedProjects && foldersArray.sharedProjects.length <= 0">
      ...
      <p>There are no projects shared with you</p>
    </div>
    <!-- -------------------NEW PROJECT -------------------- -->
    ...
    <div class="popup-div" :class="showMove?'showPopUp':''" @click="closePopup">
      <div class="popup-container" @click.stop>
        ...
        <div class="popup-head">
          <h2 class="selectDestination">Select The Destination</h2>
        </div>
        ...
        <div class="invite-button-container">
          <button @click="closePopup()" class="cancel-button">Cancel</button>
          <button @click="onClickMove()" class="update-address white">Move Here</button>
        </div>
      </div>
    </div>
...
export default {
  components: { FoldersView,ProjectSharingComponent },
  props: {
    parentId: String,
    foldersArray: Object,
    breadcrumbs: Array,
  ...
  computed: {
    ...mapState({
      ...
      projects: state => state.newProjects.projects
    })
  },
  watch: {
  ...
  methods: {
      ...
      if (!this.checkProjectNamePattern(this.projectName)) {
        ...
        this.uploadProjects({
          ...
          folder: this.foldersArray._id ? this.foldersArray._id : null
            ...
            console.log("Neo_V2project_Created")
            ...
            if (
              ...
              if(this.user && this.user.whitelabelData && this.user.whitelabelData.hasOwnProperty('redirectProject') && !this.user.whitelabelData.redirectProject){
                window.location.href = window.location.origin + "/toolv2/?project=" +  res.id;
                }else {
                  window.open(window.location.origin + "/toolv2/?project=" +  res.id);
              }
            }
    ...
    openProject(id,_role) {
      ...
      if(this.user && this.user.whitelabelData && this.user.whitelabelData.hasOwnProperty('redirectProject') && !this.user.whitelabelData.redirectProject){
          window.location.href = window.location.origin + "/toolv2/?project=" +  id;
      } else {
          window.open(window.location.origin + "/toolv2/?project=" +  id);
      }
      ...
      if(_project && _project.sptId && _project.isTemplate){
        ...
        this.createMobileProject({'projectId': _project.sptId}).then((response)=> {
          ...
          if(response && response.data && response.data._id){
            ...
            {
              if(this.user && this.user.whitelabelData && this.user.whitelabelData.hasOwnProperty('redirectProject') && !this.user.whitelabelData.redirectProject){
                window.location.href = window.location.origin + "/toolv2/?project=" +  response.data._id;
              }else {
                    window.open(window.location.origin + "/toolv2/?project=" +  response.data._id);
              }
            }
            // window.open(window.location.origin + "/tool?project=" + response.data._id);
