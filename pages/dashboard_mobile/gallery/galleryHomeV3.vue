	...
	<div class="mobile-gallery-wrapper">
		<div class="mobile-content-wrapper">
			<template v-if="transformedRenders.length === 0">
				<div class="switch-to-desktop-container">
					<div class="title">
						{{ $t("mobileSection.noRendersGalleryPageTitle") }}
					</div>
					<nuxt-link class="connect-your-desktop-btn" :to="localePath('/dashboard_mobile/link-desktop')">
						<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M12.3467 14C12.3467 14 11.92 16.5 14 18H6C8.13333 16.5 7.65333 14 7.65333 14" stroke="#E9BEB3" stroke-width="1.2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							<path d="M17 14H3C2.45 14 2 13.5519 2 13.0041V2.99585C2 2.44813 2.45 2 3 2H17C17.55 2 18 2.44813 18 2.99585V13.0041C18 13.5519 17.55 14 17 14Z" stroke="#E9BEB3" stroke-width="1.2" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
							<path d="M2 12H18" stroke="#E9BEB3" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
						</svg>
						{{ $t("buttonText.connectYourDesktop") }}
					</nuxt-link>
				</div>
				<div class="tag">{{ $t("mobileSection.seeWhatOthersCreating") }}</div>
				<mobileProjectsComponent
					
					:projects="[]"
					:rendersToShow="sampleRenders"
			...
			<template v-else>
				...
				>
					<template slot="viewAll"> <div></div> </template>
					<template slot="edit" slot-scope="slotProps"><div></div></template>
			...
			<div
				v-if="false && isRenderDetailsPopupVisible && selectedRender"
				ref="renderDrawer"
...
export default {
	...
	async mounted() {
		...
		await this.fetchRandomizeSampleRenders();
	...
	watch: {
		...
		transformedRenders(nv) {
			if(nv.length) {
				const ele = document.getElementById("mobile-nav")
				if(ele ) {
					ele.classList.add('gray-nav')
				}
			}
		}
	},
	computed: {
		...
		...mapState("Projects", ["mobileRenders"]),
		...mapState({
			user: (state) => state.User.user,
			renderPaged: (state) => state.Render.renderPaged,
			progress: (state) => state.Render.progress,
			sampleRenders: (state) => state.Render.sampleRenders,
			renderData: state => state.Dash.renders,
		}),
		...mapGetters({
...
.mobile-gallery-wrapper {
	.mobile-content-wrapper {
		...
		.switch-to-desktop-container {
			...
			background-image: url('~assets/images/mobile/connectToDeskTopBg.png');
			...
			.title {
				...
				letter-spacing: -1px;
				background: linear-gradient(94deg, $background-48 -16.49%, $background-49 72.34%);
